# Privacy Policy

**Last updated:** January 31, 2026

## Introduction

This Privacy Policy explains how we collect, use, and protect your information when you use our application (“the App”). By using the App, you agree to the practices described in this policy.

## Information We Collect

When you sign in using Google or email authentication, we may collect the following information:

- Your name
- Your email address
- Your profile picture (if provided by Google)

We do **not** collect or store passwords when you use Google Sign-In.

## How We Use Your Information

We use your information only to:

- Authenticate users
- Create and manage user accounts
- Provide access to app features
- Improve app functionality

We do **not** sell, rent, or trade your personal information.

## Third-Party Services

The App uses trusted third-party services, including:

- **Google OAuth** for authentication
- **Supabase** for backend services

These services may collect information as described in their respective privacy policies.

## Data Security

We take reasonable measures to protect your data. Authentication and user data are securely handled using industry-standard security practices.

## Data Retention

Your data is stored only as long as your account remains active. If you choose to delete your account, associated data may be removed according to our data retention policies.

## Children’s Privacy

This App is not intended for use by children under the age of 13. We do not knowingly collect personal information from children.

## Changes to This Privacy Policy

We may update this Privacy Policy from time to time. Any changes will be reflected on this page with an updated date.

## Contact Us

If you have any questions or concerns about this Privacy Policy, you can contact us at:

📧 **Email:** shikha992003@gmail.com
